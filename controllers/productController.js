...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    product.product_images.forEach((image) => {
      const imagePath = path.join(__dirname, "..", "public", image); // Assuming the path is stored relative to 'public'
...
exports.deleteManyProducts = async (req, res) => {
  try {
    ...
    if (!productIds || !Array.isArray(productIds) || productIds.length === 0) {
      return res
        .status(400)
        .json({ error: "Product IDs are required and must be an array." });
    }
    ...
    products.forEach(product => {
        product.product_images.forEach(image => {
            ...
            fs.unlink(imagePath, (err) => {
                if (err) {
                    console.error('Error deleting image for product', product._id, ':', err);
                } else {
                    console.log('Image deleted for product', product._id);
                }
            });
        });
    });
